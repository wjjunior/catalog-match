...
describe('a run over the real data', () => {
  ...
  it('bins every labelled case the posterior scored, so the low bins are the data', () => {
    const bins = summary.sections[0]?.calibration.bins ?? [];

    expect(bins.map((bin) => bin.count)).toEqual([0, 1, 1, 1, 2, 3, 4, 1, 1, 29]);
    expect(bins.filter((bin) => bin.count > 0).map((bin) => bin.precision)).toEqual(
